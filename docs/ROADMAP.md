...
- Scoping doc: [`docs/bot_kamino_xstocks_liquidator.md`](bot_kamino_xstocks_liquidator.md). Observe-first Kamino-on-Solana instrumentation path targeting xStocks weekend-reopen reserve-buffer exhaustion and liquidation-relevant events. Soothsayer is the uncertainty input; every event is logged into the V5 forward-cursor tape as research-grade C4 data; bidding is explicitly gated behind observe-only validation.
- Strategic value: (i) tests Paper 2's C4 conjecture on the real production stack rather than only retrospectively, (ii) builds the dataset Paper 2 needs anyway, (iii) generates a deployment story for Paper 2's empirical section, (iv) underwrites the grant proposal with "we already have a tape and a working consumer."
- The general-purpose Solana-liquidator path is not pursued: Kamino's Sep 2025 penalty drop to 0.1% means median liquidations are unprofitable for solo bots; the narrow xStocks lane is only interesting insofar as it produces the production tape and later validates whether the edge actually concentrates in the tail events Soothsayer flags.
...
- **First modeling exercise (cheap, fast, today):** quantify retrospective per-event EV distribution and band-exit event frequency on the existing Paper 1 dataset (5,986 weekend windows × 10 symbols). Output is a single notebook + table that motivates the observe-only build and the grant, but it is explicitly a legacy prior rather than a bidding-ready economic model. See scoping doc §10.
