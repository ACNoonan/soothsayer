...
The promise of tokenized equities is real 24/7, permissionless markets. The missing infrastructure is a defensible closed-market price.

**Don't trust the price. Verify the band.** For two-thirds of every week, a tokenized equity trades on-chain while its reference market is closed. The oracles pricing it into lending markets and AMMs hand back a stale Friday number (Chainlink Data Streams), a synthetic weekend quote (Pyth + Blue Ocean), or a dispersion figure that collapses to zero at the close (RedStone Live) — none carrying a verifiable statement of how far the price may sit from fair by Monday's open. On 13.8% of the weekends in our 12-year panel, at least one of ten tickers gapped more than 500 bps across that window.

Soothsayer inverts the oracle contract. The consumer picks a target coverage level τ; the oracle returns the **band that empirically delivers it**; and every read carries a **receipt any third party can re-derive from public data** — the exact per-(symbol, regime) scalars used, checkable against 12 years of public weekend history. It ingests upstream signals (Pyth, Chainlink, RedStone, Kamino Scope, Yahoo, Kraken) and republishes them as a band whose coverage claim is *empirically falsifiable*: over N historical windows, our 95% band contained the realised price N-in times — a number you can reproduce, and one we could be caught getting wrong. Other oracles publish a number. Soothsayer publishes a number, a band, and a coverage claim you can check.
...
The headline new evidence under M6: **per-symbol Kupiec at τ=0.95 passes for 10/10 symbols** (vs 2/10 under the prior M5 deployment), and the headline is held out on two orthogonal axes — leave-one-symbol-out coverage **0.9497 ± 0.0128** and a nested temporal holdout at **0.9504**. Full receipt: [`reports/m6_validation.md`](reports/m6_validation.md). σ̂ promotion evidence: [`reports/m6_sigma_ewma.md`](reports/m6_sigma_ewma.md). Living methodology log: [`reports/methodology_history.md`](reports/methodology_history.md).

> **Status (2026-07-17):** M6 (LWC + σ̂ EWMA HL=8) is the deployed forecaster. Validation is closed (full robustness battery + 4-DGP simulation + sample-size sweep + EWMA σ̂ promotion), and **Python↔Rust serving parity is complete (180/180 cases across both forecaster paths)**; the Anchor program serves the M5 path on devnet, with M6 on-chain enablement pending the next publisher release. The forward-tape harness is live on launchd — **N=11 held-out weekends** so far, pooled Kupiec passing at all four anchors. **Paper 1 (coverage-inversion)** has completed a full structural rewrite and a four-reviewer adversarial pass and is near arXiv submission ([`research/coverage-inversion/`](research/coverage-inversion/)); Paper 3's liquidation-policy track is in flight ([`research/liquidation-policy/`](research/liquidation-policy/)). See [`STATUS.md`](STATUS.md) for the current-state page, [`docs/INTEGRATION.md`](docs/INTEGRATION.md) for the integrator path, and [`reports/methodology_history.md`](reports/methodology_history.md) for the methodology log.
