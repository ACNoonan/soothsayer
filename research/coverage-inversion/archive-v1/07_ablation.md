...
§7.4 documents the σ̂ selection procedure (multi-test correction and the held-out forward-tape re-validation harness), §7.5 decomposes how σ̂ standardisation redistributes width across symbols, and §7.6 carries the head-to-head against a *tokenised-tracking baseline* — the continuously-updating oracle archetype that Cong et al. [cong-tokenized-2025] motivate empirically, distinct from §7.1's underlying-frozen archetype and from §6.4 / §7.2's internal-architecture comparators.
...
This failure mode is not specific to financial panels, and the fix is less obvious than it looks. Rafe and Das [socio-conformal-2026] document the same mechanism in complex survey data — standard split-conformal attaining nominal *marginal* coverage while carrying roughly 13-percentage-point weighted subgroup gaps, generated by exactly this compensation between over- and under-covered groups. The informative part for §7.2 is their **negative** result: introducing group-specific (Mondrian) thresholds made the trade-off *worse* in their setting, and a regularised comparator shrinking group thresholds toward the global quantile delivered only marginal improvement. Partitioning alone does not recover subgroup calibration. That is the reason the deployed architecture standardises the conformity score by a per-symbol $\hat\sigma_s(t)$ *before* the per-regime quantile is fit, rather than adding a per-(symbol, regime) Mondrian rung — a rung rejected during ablation development on cell-thinness grounds (§4.3). The σ̂ route recovers per-symbol calibration without spending any degrees of freedom on new cells.
...
The post-launch slice of the LWC artefact ($\text{fri\_ts} \ge 2025\text{-}12\text{-}19$) intersected with the `cex_stock_perp/ohlcv/v1` kraken_futures xstock-backed perp tape: $n = 117$ weekend-symbol observations pre-warmup, $n = 105$ after warm-up. Nine symbols (SPY, QQQ, GLD: 19 weekends each; TSLA, NVDA, GOOGL, AAPL, HOOD, MSTR: $\approx 10$ each). **TLT is excluded** — no xstock-backed perp exists for it. This is materially smaller than the $n = 1{,}730$ panel that backs §6 / §7.1 – §7.5, and the §6.4.3 / §6.4.2 backtests cited below are the powered baselines; §7.6 is a head-to-head on a powered-only-for-paired-Winkler post-launch sub-slice. See `reports/v1b_tokenised_tracking_baseline.md` for the full panel description.

### 7.6.3 Head-to-head — pooled

Soothsayer M6 LWC (one $(point, halfwidth)$ per weekend, held across the closed window) on the bake-off panel; tokenised-tracking baseline at its $\text{mon\_open}$ snapshot (the most-informed configuration). Cong et al.'s λ = 0.903 channels through this row:
...
The naive bull case for a tokenised-tracking competitor is that by Monday pre-open the tokenised perp has absorbed the weekend news flow and a band centred on it is good enough. The snapshot evolution at $\tau = 0.95$ rejects that hope:
