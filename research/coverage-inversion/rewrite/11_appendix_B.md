...
**Per-regime.** `normal` and `high_vol` are near-nominal at every anchor (`normal` 0.692 / 0.853 / 0.952 / 0.993, $n = 4{,}968$; `high_vol` 0.634 / 0.833 / 0.947 / 0.991, $n = 1{,}403$). `earnings_night` (OOS $n = 79$ nights) realises 0.798 / 0.975 / 0.987 / 1.000 — over-coverage on the small-$n$ earnings cell, the contract-favourable side of the §8 asymmetry, which should tighten as the panel accumulates (§6.5). The three cells shift between regimes rather than in total: the 19 nights recovered by the EDGAR repair of §6.5 were previously misfiled as `normal` or `high_vol`, so the pooled $n = 6{,}450$ is unchanged.
