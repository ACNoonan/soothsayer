...
**Pipeline implication.** The scryer pipelines required for Paper 4's empirical foundation (per-slot CLMM/DLMM pool state, Jito bundle attribution, BAM validator-client labelling) are simultaneously the pipelines required to evaluate Layers 2–4 as products. Standing them up early — even before Paper 4's Phase A formally begins — is therefore both a paper deliverable and a product-decision deliverable. See `research/oracle-conditioned-amm/scryer_pipeline_plan.md` for the priority order.
