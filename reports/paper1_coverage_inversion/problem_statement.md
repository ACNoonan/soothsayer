...
**(P3) Per-regime serving efficiency at deployment-tuned parameter budgets.** The deployed v2 configuration carries sixteen scalars: twelve trained per-regime conformal quantiles $q_r^{\text{LWC}}(\tau)$ on standardised residuals, four near-identity OOS-fit multiplicative bumps $c(\tau) \in \{1.000,\,1.000,\,1.079,\,1.003\}$, and zero walk-forward shifts (the v1 $\delta(\tau)$ schedule collapses under per-symbol $\hat\sigma_s(t)$ standardisation). The architecture serves a band $\bigl[\hat P_{\text{Mon},r}(s)\bigl(1 - c(\tau)\, q_r^{\text{LWC}}(\tau)\, \hat\sigma_s(t)\bigr),\,\hat P_{\text{Mon},r}(s)\bigl(1 + c(\tau)\, q_r^{\text{LWC}}(\tau)\, \hat\sigma_s(t)\bigr)\bigr]$, with the per-symbol scale $\hat\sigma_s(t)$ replacing what would otherwise be a per-symbol fallback or per-symbol Mondrian cell. The efficiency criterion is mean bandwidth at matched realised coverage *and* per-symbol Kupiec pass-rate. §7.2's head-to-head establishes empirical dominance on these criteria: v1 narrows 19–20% over Soothsayer-v0 at $\tau \le 0.95$ at indistinguishable pooled coverage; v2 over v1 takes per-symbol Kupiec from 2/10 to 10/10 at indistinguishable pooled coverage with a $+4.5\%$ pooled width tax (and is *narrower* than v1 at three of four anchors; the tax is paid only at $\tau = 0.95$). The property is an empirical-dominance statement on this sample under the paper's evaluation battery, not a theorem-shaped optimality claim against arbitrary alternatives. It is also an oracle-serving claim about width allocation at matched realised coverage; it is not a claim that the resulting band is already the optimal liquidation-policy threshold for a lending protocol.
...
3. **Distribution-free coverage.** P2 requires stationarity of the *standardised*-conditional distribution. The per-symbol $\hat\sigma_s(t)$ rescaling provides partial adaptive scaling but the standardised score's conditional stationarity is still assumed. A conformalised variant (Vovk, Barber et al.) would upgrade the statement to a finite-sample guarantee under exchangeability; we flag this as future work.
...
5. **σ̂ rule optimality.** The v2 σ̂ rule (EWMA HL=8) is justified by a pre-registered three-gate selection procedure under multiple-testing correction (§7.3), with held-out forward-tape re-validation on an architectural sibling harness. We do not claim it is optimal among all locally-weighted variants; finer half-life grids, alternative weight kernels, or hybrid rules are open future work.
