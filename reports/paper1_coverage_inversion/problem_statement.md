...
**(P3) Per-regime serving efficiency.** For each regime $r$, the oracle selects $f^\star(r)$ from a candidate set $\mathcal{F}$. The deployed selection is the *empirically preferred deployment choice* on the evaluated sample under two criteria observable post-hoc on historical data: (i) mean bandwidth at matched realised coverage, and (ii) the Christoffersen independence $p$-value as a violation-clustering diagnostic. The Christoffersen $p$-value is a sample-size- and test-form-dependent diagnostic rather than a natural decision-theoretic optimisation target, so the property is an empirical-dominance statement on this sample under this evaluation battery, not a theorem-shaped optimality claim. In our evaluation $\mathcal{F} = \{\texttt{F0\_stale},\ \texttt{F1\_emp\_regime}\}$; §7 reports the ablation matrix that justifies the hybrid regime-to-forecaster assignment. On held-out data at the deployed buffer the hybrid is *weakly dominant* over F1-everywhere on both criteria — bands tighten and the Christoffersen-independence margin widens, with no observed trade-off between the two — so the hybrid is the preferred deployment choice under the paper's evaluation objective. The in-sample motivation (mean-bandwidth dominance of F0 in `high_vol`) is preserved as the design heuristic; the out-of-sample evidence ratifies it on both criteria. This property is an oracle-serving claim about width allocation at matched realised coverage; it is not a claim that the resulting band is already the optimal liquidation-policy threshold for a lending protocol.
