...
**(P3) Per-regime serving efficiency at deployment-tuned parameter budgets.** The deployed configuration carries twenty scalars: twelve trained per-regime conformal quantiles $q_r(\tau)$, four OOS-fit multiplicative bumps $c(\tau)$, and four walk-forward-fit shifts $\delta(\tau)$ — exactly the parameter budget of the prior hybrid Oracle's `BUFFER_BY_TARGET` schedule. The architecture serves a band $\bigl[\hat P_{\text{Mon},r}(s)\bigl(1 - c(\tau') q_r(\tau')\bigr),\,\hat P_{\text{Mon},r}(s)\bigl(1 + c(\tau') q_r(\tau')\bigr)\bigr]$ at $\tau' = \tau + \delta(\tau)$, with no per-symbol fallback and no per-regime forecaster choice. The efficiency criterion is mean bandwidth at matched realised coverage. §7.2's head-to-head against the prior hybrid Oracle surface-plus-buffer Oracle establishes empirical dominance on this criterion: 19–20% narrower at indistinguishable Kupiec calibration through $\tau \le 0.95$, with the prior baseline's finite-sample tail ceiling at $\tau = 0.99$ closed at the cost of a 22% wider band. The property is an empirical-dominance statement on this sample against the prior hybrid Oracle under the paper's evaluation battery, not a theorem-shaped optimality claim against arbitrary alternatives. It is also an oracle-serving claim about width allocation at matched realised coverage; it is not a claim that the resulting band is already the optimal liquidation-policy threshold for a lending protocol.
