...
**(P3) Per-regime serving efficiency at deployment-tuned parameter budgets.** M6 carries sixteen scalars: twelve trained per-regime conformal quantiles $q_r(\tau)$ on standardised residuals, four near-identity OOS-fit multiplicative bumps $c(\tau) \in \{1.000,\,1.000,\,1.079,\,1.003\}$, and zero walk-forward shifts. The architecture serves a band $\bigl[\hat P_{\text{Mon},r}(s) - c(\tau)\, q_r(\tau)\, \hat\sigma_s(t)\, P_{\text{Fri},t},\,\hat P_{\text{Mon},r}(s) + c(\tau)\, q_r(\tau)\, \hat\sigma_s(t)\, P_{\text{Fri},t}\bigr]$, with the per-symbol scale $\hat\sigma_s(t)$ replacing what would otherwise be a per-symbol fallback or per-symbol Mondrian cell. The efficiency criterion is mean bandwidth at matched realised coverage *and* per-symbol Kupiec pass-rate. §7's ablation ladder establishes empirical dominance against the deployable alternatives evaluated on this panel — the constant-buffer baseline, the regime-stripped Mondrian, the un-standardised Mondrian comparator, and parametric GARCH-Gaussian / GARCH-$t$ baselines — at matched coverage on this sample, with the σ̂ rule itself selected from a five-variant ladder under the multiple-testing-corrected gate of §7.3. The property is an empirical-dominance statement on this sample under the paper's evaluation battery, not a theorem-shaped optimality claim against arbitrary alternatives. It is also an oracle-serving claim about width allocation at matched realised coverage; it is not a claim that the resulting band is already the optimal liquidation-policy threshold for a lending protocol.
