...
We considered eight point-estimator variants in early development (the v1 hybrid forecaster ladder; documented in `reports/methodology_history.md`). The factor-switchboard variant survived empirical pruning as the cleanest input for the residual-quantile fit; richer variants (the v1 log-log VIX / earnings-flag / long-weekend-flag layers) widened the residual quantile by absorbing structure into $\hat P$ rather than into the per-regime quantile. §7.2 shows the cleanly-pruned point estimator is what M5 deploys; the additional v1 machinery is over-engineering relative to the regime classifier.
...
A naive deployment of the trained $q_r(\tau)$ on the 2023+ OOS slice undercovers by 6–14pp at every $\tau \le 0.95$ (M2 row of §7.2.2), the same distribution-shift mechanism §7.1 identified for a global constant-buffer baseline. The fix is two deployment-tuned schedules:
