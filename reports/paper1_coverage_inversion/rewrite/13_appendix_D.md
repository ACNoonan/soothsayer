...
§7 carries the effect sizes for the three load-bearing components; this appendix carries the full tables and equations, the walk-forward δ(τ) finding, the c(τ) schedule analysis, the σ̂ selection ladder and its supporting robustness battery, the vol-index switchboard selection evidence, the width-redistribution decomposition, and the tokenized-tracking baseline — whose headline (−45% half-width, −46% Winkler at matched coverage; 7 of 9 symbols) appears in §6.3, with the construction and full grid here.
...
### D.7.7 Jupiter-mid (v5/tape) cross-check — directional confirmation on the Solana-DEX surface
