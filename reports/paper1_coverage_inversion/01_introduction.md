...
- At consumer target $\tau = 0.95$, the shipped Oracle's realised coverage on a held-out 2023+ slice (1,720 rows, 172 weekends) is **95.0%**, with Kupiec unconditional-coverage test $p$-value of **1.000** (not rejected) and Christoffersen independence $p$-value of **0.485** (not rejected). To our knowledge this is the first reported Kupiec $+$ Christoffersen pass for a weekend fair-value band on tokenized-RWA reference prices. We use $\tau = 0.95$ as the paper's headline *oracle-validation* target; we do not claim in this paper that $\tau = 0.95$ is the optimal liquidation-policy operating point for any particular lending protocol. The deployed default for protocol consumption is $\tau = 0.85$, picked on protocol-EL grounds against a Kamino flat ±300 bps benchmark (forward-referenced to Paper 3 of the trilogy; see §10.4); at that target realised coverage is $0.855$ with both tests passing.
