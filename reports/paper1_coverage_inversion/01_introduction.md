...
- **At $\tau = 0.95$ on the 2023+ slice (1,730 rows, 173 weekends):** realised $0.950$, Kupiec $p_{uc} = 0.956$, Christoffersen $p_{ind} = 0.912$, mean half-width $354.5$ bps — a 20% reduction over v1 on the same slice (CI excludes zero on width). The 12 quantiles are held-out (frozen on the 4{,}266 pre-2023 rows); the 4+4 schedules are *deployment-tuned* on the same OOS slice, matching v1's parameter budget. The strongest defensible characterisation is "deployment-calibrated operating result on a held-out conformal quantile", not "purely held-out end-to-end". A six-split walk-forward passes Kupiec at every anchor ($p$ = 0.43, 0.37, 0.36, 0.32 at $\tau \in \{0.68, 0.85, 0.95, 0.99\}$). We use $\tau = 0.95$ as the paper's primary *oracle-validation* target; the deployed default for protocol consumption is $\tau = 0.85$, picked on protocol-EL grounds (Paper 3).
