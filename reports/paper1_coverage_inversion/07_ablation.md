...
This section answers Q3 of §3.6: which components are load-bearing for the calibration claim? §7.1 stress-tests the deployable simpler baseline that strips regime structure entirely (a global constant buffer); §7.2 walks the Mondrian-by-regime ladder from the legacy comparator (Soothsayer-v0) through M5 (the unweighted Mondrian rung that immediately precedes M6) to M6 (the deployed locally-weighted Mondrian under EWMA HL=8 σ̂); §7.3 documents the σ̂ selection procedure that promoted M6's deployed σ̂ rule, including the multiple-testing exposure and the held-out forward-tape re-validation; §7.4 decomposes how σ̂ standardisation redistributes width across symbols. Soothsayer-v0 is the legacy hybrid forecaster ladder (factor switchboard, log-log VIX standardisation, per-symbol vol index, earnings flag, long-weekend flag) wrapped in a per-(symbol, regime, claimed-quantile) calibration surface plus a 4-scalar `BUFFER_BY_TARGET` schedule, fully documented in `reports/methodology_history.md`.
...
| 0.68 | M6 deployed | 0.693 | 130.8 | 0.264 | 0.244 |
| 0.85 | constant buffer (train-fit) | 0.731 | 140.0 | 0.000 | 0.000 |
| 0.85 | M6 deployed | 0.855 | 213.6 | 0.565 | 0.403 |
| 0.95 | constant buffer (train-fit) | 0.897 | 272.0 | 0.000 | 0.013 |
| 0.95 | M6 deployed | 0.950 | 370.6 | 0.956 | 0.603 |
| 0.99 | constant buffer (train-fit) | 0.984 | 695.0 | 0.018 | 0.927 |
| 0.99 | M6 deployed | 0.990 | 635.0 | 0.942 | $\approx 1.0$ |

The training-fit constant buffer **catastrophically undercovers** at every $\tau \leq 0.95$ (deficits $-14.2 / -12.0 / -5.4$pp, all rejecting Kupiec at $p_{uc} < 10^{-6}$) — non-stationarity: $b(\tau)$ is calibrated on a 2014–2022 window calmer than the 2023+ holdout. **The deployable constant-buffer baseline does not deliver coverage on the holdout; M6 does.** This is the *adaptivity-to-distribution-shift* contribution of the regime model + per-symbol σ̂.
...
| $\tau$ | matched $b$ | matched-CB hw (bps) | M6 hw (bps) | $\Delta$ width (M6 − CB) |
|---:|---:|---:|---:|---|
...
**M6 is narrower than a coverage-matched (oracle-fit) constant buffer at $\tau \in \{0.85, 0.95\}$.** The σ̂ standardisation that redistributes width across symbols (§6.4.4) reaches sufficient sharpness on the equity-heavy panel that M6 outperforms even the oracle-fit constant buffer at the headline anchor. M6 is wider only at $\tau \in \{0.68, 0.99\}$, where the cell-conditional regime structure carries genuine information. The regime model + σ̂ buy *both* coverage on a non-stationary distribution *and* mean-width sharpness at $\tau \in \{0.85, 0.95\}$ against the strongest non-modelled comparator.

### 7.1.3 Per-regime decomposition

The pooled half-width at $\tau = 0.95$ hides a regime-conditional structure. M6 pooled: $370.6$ bps; per-regime $300.1 / 334.7 / 603.7$ bps (normal / long_weekend / high_vol). The matched-CB at $\tau = 0.95$ delivers $395.6$ bps in *every* regime by construction (one global multiplier). The regime model concentrates width in `high_vol` ($+52\%$ vs CB at $603.7$ vs $395.6$) where it earns $-1.1$pp coverage; releases width in `normal` ($-24\%$ vs CB at $300.1$ vs $395.6$) without losing coverage. The constant buffer's pooled coverage is "average right but worst where it matters most": violations concentrate in high-vol weekends — exactly where a downstream lending protocol incurs the largest mark-to-market gap. The regime model's contribution is **(i) calibration through distribution shift** and **(ii) tail-protection in `high_vol` weekends** at sharper *normal*-regime widths than the constant buffer.

## 7.2 Mondrian-by-regime ladder — through M5 to M6

§7.1 ruled out a deployable baseline stripping regime structure. The natural follow-up: which architectural ingredients of M5 → M6 are load-bearing? The answer separates into (a) the M5 contribution (the per-regime conformal quantile + OOS-fit $c(\tau)$ replacing Soothsayer-v0's hybrid ladder), and (b) the M6 contribution (per-symbol σ̂ standardisation closing the per-symbol bimodality of an unweighted Mondrian fit on heterogeneous tails).
...
- **M5.** M2 + per-target multiplicative bump $c(\tau)$ tuned on OOS + walk-forward $\delta(\tau)$ shift schedule. **The Mondrian-without-σ̂ rung.** Twelve trained scalars + 4 OOS-fit + 4 walk-forward-fit = 20 scalars.
- **M6-K26.** M5 + per-symbol pre-Friday σ̂ on a 26-weekend trailing window (the locally-weighted variant before σ̂ promotion).
- **M6.** M6-K26 with σ̂ rule swapped to EWMA HL=8 (the §7.3 selection-validated variant). **The deployed architecture.** Twelve trained scalars + 4 OOS-fit $c(\tau)$ + 0 walk-forward (collapses) = 16 scalars.

The σ̂ rule for M6 is itself a soft constant (half-life 8 weekends, ≥8-past-obs warm-up); it is not an additional fit-against-OOS scalar.
...
| 0.68 | M5 deployed | 0.735 | 137.5 | 0.000 | 0.339 | (M5 fails 8/10) |
| 0.68 | M6-K26 | 0.687 | 132.6 | 0.515 | 0.102 | — |
| 0.68 | M6 deployed | 0.693 | 130.8 | 0.264 | 0.244 | 10/10 |
| 0.85 | Soothsayer-v0 | 0.856 | 251.4 | 0.477 | 0.182 | — |
| 0.85 | M5 deployed | 0.887 | 235.1 | 0.000 | 0.424 | — |
| 0.85 | M6 deployed | 0.855 | 213.6 | 0.565 | 0.403 | — |
| **0.95** | Soothsayer-v0 | 0.950 | 443.5 | 0.956 | 0.483 | — |
| **0.95** | M2 | 0.910 | 272.7 | $8.2{\times}10^{-12}$ | 0.396 | — |
| **0.95** | **M5 deployed** | **0.950** | **354.6** | **0.956** | **0.921** | **2/10** |
| **0.95** | **M6-K26** | **0.950** | **385.3** | **0.956** | **0.275** | **10/10** |
| **0.95** | **M6 deployed** | **0.950** | **370.6** | **0.956** | **0.603** | **10/10** |
| 0.99 | Soothsayer-v0 | 0.972 | 522.8 | 0.000 | 0.897 | — |
| 0.99 | M5 deployed | 0.990 | 677.7 | 0.942 | 0.344 | — |
| 0.99 | M6 deployed | 0.990 | 635.0 | 0.942 | $\approx 1.0$ | — |

**The two structural moves are visible.** M5 over Soothsayer-v0: **20% narrower** at $\tau = 0.95$ (354.6 vs 443.5 bps; CI $-23.9\%$ to $-15.6\%$) at indistinguishable pooled coverage, and closes Soothsayer-v0's tail ceiling at $\tau = 0.99$. M6 over M5: **per-symbol Kupiec pass-rate 2/10 → 10/10** at indistinguishable pooled coverage, with a $+4.5\%$ pooled half-width tax (CI on $\Delta$hw at $\tau = 0.95$ from `m5_vs_m6_bootstrap.csv` is $[+12.3, +49.0]$ bps for M5 → M6-K26; the M5 → M6 EWMA HL=8 delta is the M5 → M6-K26 delta minus the K=26 → EWMA HL=8 narrowing of $-14.8$ bps, giving M5 → M6 ≈ $+16$ bps or $+4.5\%$). The Christoffersen $p$ at $\tau = 0.95$ on the 2023 split passes for all three (M5: 0.921; M6-K26: 0.275; M6: 0.603) — but the K=26 baseline of M6 introduced split-date Christoffersen rejections at the 2021/2022 split anchors that M5 didn't have, motivating the §7.3 σ̂ selection. **§7.4 documents how the σ̂ standardisation redistributes the per-symbol width that M5 was leaving on the table.**

### 7.2.3 Walk-forward δ-shift schedule — collapses under M6

Under M5, a plain $c(\tau)$ fit to the pooled OOS slice produced per-split realised coverage that scattered around nominal — passed Kupiec on the pooled fit by construction, but undercovered in roughly half the splits. M5 added a four-scalar walk-forward $\delta(\tau) = \{0.05,\, 0.02,\, 0.00,\, 0.00\}$ to push every split above nominal. Under M6 the same walk-forward sweep over $\delta \in \{0.00, \dots, 0.07\}$ selects $\delta(\tau) \equiv 0$ — per-symbol scale standardisation tightens cross-split realised-coverage variance enough that the structural-conservatism shim is no longer load-bearing. M5 vs M6 at the deployed (post-walk-forward) schedule:

| $\tau$ | M5 deployed (with δ) | M6 deployed (δ=0) | M6 advantage |
|---:|---|---|---|
...
At three of four anchors M6 is *narrower* than M5 with a stricter per-symbol calibration constraint. At $\tau = 0.95$ — the headline anchor — M6 is $+4.5\%$ wider and trades that for the per-symbol fix. The M5 over-conservatism at $\tau \in \{0.68, 0.85\}$ (Kupiec rejecting because $\delta$-shift pushed coverage above nominal) is what σ̂ standardisation removes.

### 7.2.4 The per-symbol-bimodality fix (M5 → M6)
...
1. **Gate 1 — split-date Christoffersen.** No (split × $\tau$) cell rejecting at uncorrected $\alpha = 0.05$ across the four split anchors {2021, 2022, 2023, 2024} × four $\tau$ anchors. Motivation: the K=26 baseline introduced 2021/2022 split-date Christoffersen rejections at $\tau = 0.95$ that M5 did not have; we want a σ̂ rule that closes those without re-introducing them elsewhere.
2. **Gate 2 — per-symbol Kupiec.** Pass-rate 10/10 at $\tau = 0.95$. Motivation: the M6 architecture exists to fix the M5 per-symbol bimodality; any σ̂ rule that loses this property is rejected.
...
- **K=26 baseline.** Trailing 26-weekend window, equal-weight (the M6-K26 from §7.2).
...
§7.1.3 attributed the constant-buffer premium to `high_vol`; §7.2 attributed the M5-over-Soothsayer-v0 narrowing to the per-regime conformal lookup replacing Soothsayer-v0's hybrid forecaster ladder; §7.4 attributes the M5 → M6 width redistribution to per-symbol σ̂.

**M5 vs Soothsayer-v0 per-regime decomposition at $\tau = 0.95$:**

| regime | $n$ | M5 hw | Soothsayer-v0 hw | Soothsayer-v0 premium |
|---|---:|---:|---:|---|
...
Soothsayer-v0 earns its OOS calibration **primarily by overwidening normal-regime weekends** (the 67% of the panel) at $+43.9\%$ wider for $+0.7$pp coverage — the Soothsayer-v0 forecaster ladder is cosmetic on top of `regime_pub`.

**M5 vs M6 per-class decomposition at $\tau = 0.95$.** Where M5 paid its $+4.5\%$ pooled width tax over Soothsayer-v0, M6 redistributes that width *across symbols within a regime* via σ̂:

| class | $n$ | M5 hw at $\tau=0.95$ | M6 hw at $\tau=0.95$ | Δ |
|---|---:|---:|---:|---|
...
Under M5, every symbol within a regime received the same $c(\tau) \cdot q_r(\tau) \cdot p_{\text{Fri}}$ width — the "common multiplier on heterogeneous tails" failure of §6.4.1. Under M6, the σ̂ multiplier is per-symbol, so heavy-tail equities (HOOD, MSTR, TSLA) receive bands that match their relative residual scale, and the over-covered defensive class (GLD, TLT) receives narrower bands without losing coverage. **The M5 → M6 transition is a width-redistribution at neutral pooled coverage**, not a free-lunch claim and not a mean-width improvement on the panel-aggregate scale.

The wire format (`PriceUpdate` Borsh) is preserved across the migration M5 → M6: the swap reserves a new `forecaster_code` (M5: 2 = Mondrian; M6: 3 = LWC) but the decoder, the field schema, and the §3.4 property contract are unchanged. M6 is the deployable architecture this paper validates; the §10 candidate architectures (cross-sectional common-mode partial-out, full-distribution conformal, sub-regime granularity) target the residual rejections (§6.3.6 cross-sectional within-weekend correlation; §9.4 per-symbol Berkowitz on TSLA/TLT) that σ̂ standardisation does not reach.
