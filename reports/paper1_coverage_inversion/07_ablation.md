...
This section answers Q3 of §3.6: which components are load-bearing for the calibration claim? §7.1 stress-tests the deployable simpler baseline that strips regime structure entirely (a global constant buffer); §7.2 compares M5 against an internal predecessor we refer to as **the prior hybrid Oracle** — a hybrid forecaster ladder (factor switchboard, log-log VIX standardisation, per-symbol vol index, earnings flag, long-weekend flag) wrapped in a per-(symbol, regime, claimed-quantile) calibration surface plus a 4-scalar `BUFFER_BY_TARGET` schedule, with the same parameter budget as M5's 4-scalar $c(\tau)$. It is the deployed Soothsayer methodology M5 was designed to replace; its full forecaster ladder is documented in `reports/methodology_history.md` and the empirical comparison happens in §7.2.
...
## 7.2 Mondrian conformal-by-regime (the deployed M5 architecture)

§7.1 ruled out a deployable baseline stripping regime structure. The natural follow-up: does the prior hybrid Oracle's forecaster machinery (factor switchboard, log-log VIX, per-symbol vol index, earnings flag, long-weekend flag) plus the per-target buffer schedule earn its place against a Mondrian split-conformal quantile fit per regime?
...
- **M5.** M2 + per-target multiplicative bump $c(\tau)$ tuned on OOS — twelve trained scalars + four OOS, matching the prior hybrid Oracle's parameter budget. **The deployed variant.**

### 7.2.2 OOS pooled results

| $\tau$ | method | realised | hw (bps) | $p_{uc}$ | $p_{ind}$ |
|---:|---|---:|---:|---:|---:|
| 0.68 | the prior hybrid Oracle | 0.680 | 136.1 | 0.984 | 0.645 |
...
**M5 with the δ-shift schedule passes Kupiec at every anchor on walk-forward at 25–33% narrower mean half-width than the prior hybrid Oracle through $\tau \leq 0.95$.** At $\tau = 0.99$, M5 trades width for target attainment.

### 7.2.4 Where the prior hybrid Oracle premium goes

§7.1.3 attributed the constant-buffer premium to `high_vol`; §7.2 shifts the question. M5 vs prior-baseline $\tau = 0.95$ decomposition:

| regime | $n$ | M5 hw | the prior hybrid Oracle hw | the prior hybrid Oracle premium |
|---|---:|---:|---:|---|
...
the prior hybrid Oracle earns its OOS calibration **primarily by overwidening normal-regime weekends** (the 67% of the panel) at $+43.9\%$ wider for $+0.7$pp coverage — the prior hybrid Oracle forecaster ladder is cosmetic on top of `regime_pub`. Berkowitz and DQ on M5's walk-forward PITs reject ($\text{LR} = 173.1$; DQ at $\tau = 0.95$, $p = 5.7 \times 10^{-6}$) — same diagnosis as §6.3.1, full-distribution conformal remains the next-generation target.

§7.1 ruled out a deployable baseline that strips regime structure. §7.2 isolates the deployable baseline that keeps `regime_pub` and replaces the prior hybrid Oracle's forecaster machinery: 19–20% narrower than the prior hybrid Oracle at indistinguishable coverage on OOS at $\tau \leq 0.95$; 25–33% narrower on walk-forward; hits nominal $\tau = 0.99$ where the prior hybrid Oracle ceilings; shares the prior hybrid Oracle's per-anchor-only profile on density tests. The wire format (`PriceUpdate` Borsh) is preserved across the migration to M5: the swap changes only the published *values*, not the decoder or the §3.4 property contract.
