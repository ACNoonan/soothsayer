...
At consumer target $\tau = 0.99$, the served Oracle delivered $97.7\%$ realised coverage on the OOS slice — a $1.3$pp shortfall and a Kupiec rejection ($p_{uc} \approx 0$, LR $= 22.2$). The 99% band is structurally limited by the per-(symbol, regime) calibration-window size. Our log-log residual model is fit on a rolling $156$-weekend window; reliably resolving the $1\%$ tail of that distribution requires more observations than any one bucket carries. The pooled-surface fallback partially mitigates this but does not eliminate it: the pooled bucket draws on $\sim 600$ observations per regime, which is still tight for 1-in-100 events. The 2026-04-25 extension of the bounds grid from a $0.995$ cap to $\{0.997, 0.999\}$ lifted realised at $\tau = 0.99$ from $0.972$ to $0.977$ but did not close the gap, confirming that the limit is sample-size-driven rather than grid-spacing-driven (see `reports/v1b_extended_grid.md`).

The ceiling has a quantitative magnitude bound. The exceedance-magnitude diagnostic of §6.4.1 reports that the 40 missed-coverage events at $\tau = 0.99$ have median breach $72$ bps (of Friday's close) and maximum breach $796$ bps. This is the failure profile a protocol consuming the $\tau = 0.99$ band would experience: in the $2.3\%$ of weekends where the band misses, the realised price lands a median of $0.7\%$ outside the band, with worst-case excursion of approximately $8\%$. Across the four anchor τ in §6.4.1 the maximum breach shrinks monotonically in $\tau$ ($2{,}339 \to 2{,}150 \to 1{,}415 \to 796$ bps for $\tau = 0.68 \to 0.85 \to 0.95 \to 0.99$), as wider bands catch larger shocks — this is the qualitative property a coverage-claiming oracle should have.

The ceiling is therefore better characterised as a *level-attribution failure* than as a tail-blowup failure: the served band rate is wrong by $1.3$pp at the highest anchor, and the Kupiec test correctly identifies this, but the resulting coverage-loss events are bounded in magnitude rather than open-ended. This is not a failure of the coverage-inversion primitive — it is a finite-sample artefact of the empirical quantile estimator at the tail of the calibration window. A conformalised upgrade (see §9.4) would report finite-sample coverage bounds instead, at the cost of assumption strength; protocols that require true $\tau = 0.99$ behaviour today should layer an exogenous tail-widening rule on top of the served band, calibrated to their own loss function.
