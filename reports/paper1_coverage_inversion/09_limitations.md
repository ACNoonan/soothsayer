...
At consumer target $\tau = 0.99$, the served Oracle delivers realised coverage $0.977$ — a $1.3$pp shortfall by Kupiec ($p_{uc} \approx 0$, LR $= 22.2$), but with quantitatively bounded impact. The exceedance-magnitude diagnostic of §6.4.1 reports that the 40 missed-coverage events at $\tau = 0.99$ have median breach $72$ bps (of Friday's close) and maximum $796$ bps; across the four anchor $\tau$ the maximum breach shrinks monotonically in $\tau$ ($2{,}339 \to 2{,}150 \to 1{,}415 \to 796$ bps for $\tau = 0.68 \to 0.85 \to 0.95 \to 0.99$), as wider bands catch larger shocks — the qualitative property a coverage-claiming oracle should have. The shortfall is therefore better characterised as a *level-attribution failure with bounded magnitude* than as an open-ended tail risk: the served band rate is wrong by $1.3$pp at the highest anchor, and Kupiec correctly identifies this, but a protocol consuming the $\tau = 0.99$ band experiences a worst-case $\sim 8\%$ price-mismatch on missed events, with median $\sim 0.7\%$.
...
## 9.6 Hybrid regime policy is the joint argmin, not a binary necessity

In-sample evidence (matched-realised-coverage sharpness dominance of F0 in `high_vol`) motivated the hybrid serving policy. Out-of-sample at the deployed buffer, the hybrid's mean-coverage effect is statistically indistinguishable from F1-everywhere ($\Delta\text{cov} = -0.1$pp, CI $[-0.7, +0.4]$). At the deployed buffer both F1-everywhere and the hybrid pass Kupiec and Christoffersen at $\alpha = 0.05$; the hybrid is *not* a binary verdict-flip on Christoffersen independence — it is the joint argmin over (mean bandwidth, Christoffersen margin), winning on both axes simultaneously: bands tighten by $2.6\%$ ($467.9 \to 456.0$ bps) and the Christoffersen $p_{ind}$ margin lifts from $0.221$ to $0.485$. We disclose this softening of the original "Christoffersen independence is load-bearing" framing because at an earlier stage of v1 development (under a scalar 0.025 buffer) F1 + buffer did fail Christoffersen independence and the hybrid was strictly necessary; at the deployed buffer the hybrid is a margin-and-sharpness improvement rather than a defect-fix. The historical record of the buffer-tuning sequence is preserved in `reports/methodology_history.md`.
...
We do not report a systematic numerical comparison of Soothsayer's served bands against Pyth, Chainlink Data Streams, or RedStone Live on a matched evaluation window. The conceptual comparison in §1.1 (stale-hold, dispersion CI, undisclosed methodology) is qualitative; a numerical benchmark would require either access to incumbent-internal historical bands or a reconstruction of incumbent values from on-chain feeds. Our team has partial Chainlink v10 / v11 reconstruction infrastructure in-repo that was used for a sanity-check comparison (`reports/v1_chainlink_bias.md`, `reports/v1b_chainlink_comparison.md`) and a Pyth Hermes-based comparison (`reports/v1b_pyth_comparison.md`) but neither is at the rigor required for a research-paper-grade benchmark. The RedStone-side equivalent has additional limits documented in §9.8.1; a forward-cursor tape against the public RedStone gateway began continuous capture on 2026-04-26 (`scripts/run_redstone_scrape.py`, persisted to `data/processed/redstone_live_tape.parquet`) and will be reportable once it accumulates a paper-grade weekend sample. We flag the matched-window comparator dashboard as future work (§10) — specifically, an AFT-community-facing public comparator computed on a common historical window.
