...
**Partial-only numerical incumbent benchmark.** §6.5 reports a partial comparison against regular Pyth (265 obs) and Chainlink Data Streams v10 + v11; the three caveats (wide sample CIs, large-cap normal-regime composition, consumer-supplied wrap multipliers) prevent reading any matched-bandwidth observation as a head-to-head loss. RedStone Live is excluded — the public REST gateway carries no calibration object on the wire, prices underlier tickers (SPY, QQQ, MSTR) rather than the SPL xStock tokens consumers hold, returns empty on TSLA/NVDA/HOOD/GOOGL, and retains only 30d so only forward-collected weekends can enter. A forward-cursor tape began capture on 2026-04-26 and will be reportable once a paper-grade sample accumulates. Pyth Pro / Blue Ocean is excluded on access and window grounds.
