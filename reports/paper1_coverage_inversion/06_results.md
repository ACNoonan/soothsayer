...
**Per-regime.** `normal` and `high_vol` are near-nominal at every anchor (`normal` 0.691 / 0.853 / 0.953 / 0.992; `high_vol` 0.638 / 0.837 / 0.948 / 0.992). `earnings_night` (OOS $n \approx 60$ nights) realises 0.767 / 0.967 / 0.983 / 1.000 — a contract-favourable over-coverage on the small-$n$ earnings cell, the same one-sided asymmetry framed in §9.3, which tightens as the panel accumulates. The overnight panel uses raw (unadjusted) opens, so ex-dividend mornings enter as small systematic down-gaps; coverage holds at every $\tau$ without correction, and a dividend-adjusted rebuild is deferred to §10 pending an upstream ex-dividend feed (§9.8).
