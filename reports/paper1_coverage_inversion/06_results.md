...
*Three caveats apply to both subsections below.* (i) Sample-size CIs are wide: binomial 95% CI on $\hat p = 0.95$ is $[0.92, 0.98]$ at $n = 265$ and tighter only on the larger Chainlink panels. (ii) Sample composition is skewed toward large-cap, low-realised-volatility tickers and *normal*-regime weekends, so any matched-bandwidth comparison should be read against Soothsayer's normal-regime OOS half-width (417.7 bps), not the pooled figure (456 bps). (iii) The wrap multipliers below ($k = 50$ for Pyth, $k = 3.2\%$ for Chainlink) are *consumer-supplied* — neither incumbent publishes them, so the consumer back-fits the multiplier with no audit trail and no per-symbol stratification.
