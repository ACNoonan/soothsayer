...
![The joint-tail distribution at $\tau = 0.95$: empirical $P(k_w = k)$ over the 173 OOS weekends (blue bars) against the $\mathrm{Binom}(10, 1{-}\tau)$ independence strawman (vermilion) and the fitted Student-$t$ copula ($\hat\nu = 6.04$, empirical $\hat R$; grey dashed), log scale. The empirical mass at $k \ge 3$ sits a factor $\sim 3$–$10\times$ above independence — the variance overdispersion in one picture — while remaining inside the t-copula envelope from below. The reserve-guidance threshold $k^\ast = 3$ and the single $k_w = 9$ weekend (2024-08-05 BoJ unwind; Fig. \ref{fig:boj-anatomy}) are marked; under independence that weekend is a $\sim 10^{-4}$ event, under the t-copula $\sim 10^{-3}$. No incumbent oracle publishes this distribution.\label{fig:kw-distribution}](figures/fig10_kw_distribution.pdf)
...
![Per-symbol calibration at $\tau = 0.95$ on the 2023+ OOS slice. Each symbol's violation rate under the deployed architecture (blue circles) sits within the binomial 95% confidence band around the nominal $0.05$ rate (shaded); symbols are ordered by their realised PIT variance $\hat\sigma^2_z$. The unweighted Mondrian comparator's rates on the same cells (grey ×) expose the §7.2 bimodality: heavy-tail tickers (TSLA, MSTR, HOOD) under-cover at $11.6$–$15.6\%$ violation rates while low-vol tickers (TLT, SPY, AAPL, QQQ, GLD) over-cover at $\le 0.6\%$ — pooled-coverage parity through compensating per-symbol biases. The deployed distribution is unimodal and centred on nominal.\label{fig:per-symbol}](figures/fig4_per_symbol.pdf)
...
**Status at submission:** $N = 11$ forward weekends (`reports/m6_forward_tape_11weekends.md`, 2026-05-01 → 2026-07-10) with $n = 110$ symbol-rows; pooled realised coverage $0.746 / 0.845 / 0.964 / 1.000$ at $\tau \in \{0.68, 0.85, 0.95, 0.99\}$ (no under-coverage), with pooled Kupiec passing at all four anchors ($p = 0.13 / 0.89 / 0.49 / 0.14$) and pooled Christoffersen passing at the three anchors with observed violations ($p = 0.79 / 0.48 / 0.36$). $N = 11$ clears the harness's own preliminary banner ($N \ge 4$); it remains just under the $N \ge 13$ threshold for cumulative pooled Christoffersen power. Per-symbol Kupiec at $\tau = 0.95$ passes 10/10 on the forward tape (low-power at $n = 11$ per symbol, but directionally consistent with the in-sample 10/10; the M5 in-sample baseline was 2/10). The present submission's held-out backbone is §6.3.3's leave-one-symbol-out CV plus per-symbol Kupiec, split-date sensitivity, and four-year calendar sub-period stability; the forward-tape harness becomes load-bearing as $N$ accumulates.
...
§6.2–§6.7 validate the architecture on the **weekend** gap (the ~65-hour Friday-close → Monday-open window). The closed-market problem it addresses, however, recurs every trading night: the ~17-hour close → next-open window when the US venue is shut but overseas sessions and index futures continue to price the underlier. If calibration-transparency is a property of the *method* rather than an artefact of the weekend, the same architecture should calibrate on overnight gaps. We test this directly.

We rebuild the panel with a single change — the gap selector admits consecutive-trading-day pairs (`gap = 1`) rather than Friday→Monday pairs — yielding **22,624 overnight rows across 2,412 weeknights** on the same ten tickers, 2014-01-16 → 2026-04-23 (≈3.8× the weekend panel). Every downstream component (factor-adjusted point, per-symbol $\hat\sigma_s(t)$, Mondrian split-conformal, OOS-fit $c(\tau)$) is gap-length-agnostic and reused unchanged; the §6.2–§6.7 weekend results are byte-identical under the default selector. The regime set is re-derived for the overnight cadence (§4.3): `long_weekend` has no analog and is dropped, `high_vol` is retained with a 252-trading-day VIX lookback, and an `earnings_night` regime is added (§4.3, calendar-conditioned coverage).

**Pooled OOS conditional coverage** (overnight; TRAIN < 2023-01-01, 16,093 rows; OOS ≥ 2023-01-01, 6,450 rows × 645 nights; post-dividend-adjustment run, `reports/active/overnight_calibration_firstread.md` @ 2026-06-25):
...
**Kupiec and Christoffersen pass at every served anchor** — the same headline property as the weekend panel (§6.3.1), reproduced on a different and ≈3.8× larger panel, with the OOS-fit $c(\tau)$ collapsing to ≈1.0 (the overnight bands need essentially no post-hoc widening to calibrate).
