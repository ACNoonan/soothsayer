...
Quantile fit on pre-2023 weekends ($n = 4{,}186$); served on the 2023+ slice (1,730 rows × 173 weekends). The 12 trained per-regime quantiles $q_r(\tau)$ are held-out. The 4-scalar $c(\tau)$ schedule is OOS-fit, but **the $\tau = 0.95$ headline is held out on two orthogonal axes** — *temporally* via a nested 2023-only TUNE / 2024–2026 EVAL split (§6.3.3.1) and on *symbol* via leave-one-symbol-out CV (§6.3.3) — both of which evaluate $c(0.95)$ on data its fit never touched. The pooled $0.950$ at $\tau = 0.95$ reported below is the deployment-tuned read on the full OOS slice; the held-out read is reported in §6.3.3 alongside it. The walk-forward $\delta(\tau)$ schedule is identically zero — per-symbol scale standardisation tightens cross-split realised-coverage variance enough that no structural-conservatism shift is required (§4.5). §9.3 carries the residual provenance disclosure (now substantially contracted; only the lower-$\tau$ over-coverage in held-out mode remains, a one-sided contract-favourable failure mode rather than a coverage-contract violation).
...
**Variance-overdispersion lead.** Empirical $k_w$ over-disperses $2.34\times$ vs Binom independence and $0.63\times$ vs a Student-$t$ copula at $\tau = 0.95$, with the same pattern at every served $\tau$: emp/Binom variance ratio is $\{2.34, 2.31, 2.34, 2.35\}$ at $\tau \in \{0.68, 0.85, 0.95, 0.99\}$ — independence is robustly misspecified by a uniform factor across the served $\tau$ range, indicating systematic joint-tail dependence rather than a $\tau$-specific calibration anomaly. Versus the t-copula, emp/t variance ratio is $\{0.83, 0.54, 0.63, 0.73\}$ — the empirical $k_w$ distribution sits inside a properly specified joint-baseline envelope (Wasserstein-1 distance to the t-copula $\le 0.180$ at every $\tau$, less than half the distance to Binom independence). Marginal calibration is preserved (mean $k_w$ matches every reference to $0.003$); the dispersion shows up in the *variance* and the *upper tail*, and the structure is brackets-bounded: empirical sits between independence and a homogeneous t-copula. The same pattern is more visible at $\tau = 0.85$ where absolute counts are larger: emp/Binom overdispersion $2.31\times$, $P(k_w \ge 5) = 5.78\%$ vs Binom $0.99\%$, and the worst-observed weekend has all 10 symbols breaching simultaneously.

The Gaussian copula sits closest in Wasserstein-1 at $\tau = 0.95$ (0.134) and the t-copula at $\tau = 0.85$; at deeper tails ($\tau = 0.99$) the copulas pull ahead of Binom by roughly $2\times$ in WS-1 distance. The joint dependence is real (mean off-diagonal $\hat R = 0.36$), has parametric structure (a t-copula at $\hat\nu \approx 6$ brackets the empirical from above), and is misspecified by Binom by a uniform $\sim 2.3\times$ variance factor — a *systematic-dependence* finding, not a $\tau$-specific anomaly. Cross-sectional within-weekend dependence ($\hat\rho_\text{cross} = 0.41$) is respected by all reported CIs via weekend-block bootstrap; temporal autocorrelation across weekends is empirically null — lag-1, lag-2, lag-4 $\hat\rho$ on weekly $k_w$ lie in $[-0.07, 0.03]$ across served $\tau$ — so a moving-block-bootstrap with block lengths $L \in \{4, 8, 13\}$ produces CIs within Monte Carlo noise of the $L = 1$ (i.i.d.-weekend) baseline (`reports/tables/paper1_b5_kw_block_bootstrap.csv`).
...
**Operational consumer guidance — the reserve-guidance threshold $k^\ast = 3$ at $\tau = 0.95$ is conservative against a properly specified joint baseline, not just against independence.** A 10-symbol xStock-style portfolio at $\tau = 0.95$ should reserve against an empirical 99th-percentile of $\approx 5$ simultaneous breaches, not the binomial $\approx 2$; the t-copula reference puts the same 99th percentile at $\approx 6$. At $\tau = 0.85$ the empirical 99th percentile is $\approx 7$ vs binomial $\approx 4$. A consumer using $k^\ast = 3$ isn't relying on a number calibrated against an obvious strawman; it's using a number that holds up against the kind of joint-volatility model a CCAR-style risk team would actually run. **No incumbent oracle (Pyth, Chainlink Data Streams, RedStone, Kamino Scope) publishes this distribution**; this is the joint-tail measurement layer that calibration-transparency makes possible.
