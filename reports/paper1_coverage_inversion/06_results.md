...
**Status at submission:** $N = 5$ forward weekends (`reports/m6_forward_tape_5weekends.md`, 2026-05-01 → 2026-05-29) with $n = 50$ symbol-rows; pooled realised coverage $0.76 / 0.82 / 0.96 / 1.00$ at $\tau \in \{0.68, 0.85, 0.95, 0.99\}$ (no under-coverage), with pooled Kupiec passing at all four anchors ($p = 0.21 / 0.56 / 0.74 / 0.32$). $N = 5$ clears the harness's own preliminary banner ($N \ge 4$); it remains under the $N \ge 13$ threshold for cumulative pooled Christoffersen power, so per-anchor independence statistics are persisted but not reported inline. Per-symbol Kupiec at $\tau = 0.95$ passes 10/10 on the forward tape (low-power at $n = 5$ per symbol, but directionally consistent with the in-sample 10/10). The present submission's held-out backbone is §6.3.3's leave-one-symbol-out CV plus per-symbol Kupiec, split-date sensitivity, and four-year calendar sub-period stability; the forward-tape harness becomes load-bearing as $N$ accumulates.
...
**Pooled OOS conditional coverage** (overnight; TRAIN < 2023-01-01, 16,094 rows; OOS ≥ 2023-01-01, 6,450 rows × 645 nights):
