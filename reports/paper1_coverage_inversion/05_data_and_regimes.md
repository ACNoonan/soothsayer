...
The split is temporally disjoint at the surface-construction level. The calibration surface $S^f$ is built from the 4,266 calibration-set rows (per-(symbol, regime, forecaster, claimed-quantile) cells) and held *fixed* throughout the OOS evaluation. Every OOS row is served through the frozen surface: no parameter, threshold, or grid point is updated using OOS data. The per-target buffer schedule (§4.3) was tuned on this same OOS slice using the methodology of `reports/v1b_buffer_tune.md` — this is disclosed as a sample-size-1 calibration step in §9.4 and partially closed in §6 by the walk-forward stability evidence (`reports/v1b_walkforward.md`) showing the deployed buffer values land at the cross-split mean.
