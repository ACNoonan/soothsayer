...
A separate post-hoc tertile labeler tags each weekend by realised-move z-score (calm / normal / shock) using $|z| = |\log(P_t / P_{t^-})| / \hat\sigma^{\text{20d}}_t$. This `realized_bucket` is *not* a regime in the §3.1 sense — it depends on the realised target — and is used only for diagnostic stratification (the shock-tertile coverage ceiling reported in §9.1). It never enters $\rho$ or the calibration surface.
...
| Held-out OOS (2023-01 → 2026-04) | 1{,}730 | 173 | All 10 |

The split is temporally disjoint at the conformal-quantile-construction level. The 12 trained per-regime quantiles $q_r(\tau)$ of the deployed v2 / M5 architecture (§4.2) are fit from the 4,266 calibration-set rows and held *fixed* throughout the OOS evaluation. Every OOS row is served through the frozen quantiles: no information from the 2023+ slice enters the trained quantile table. The four $c(\tau)$ bumps and four $\delta(\tau)$ shifts (§4.3) are deployment-tuned on this same OOS slice — this is disclosed as a sample-size-one OOS-fit step in §9.3 and partially closed by the six-split walk-forward (§7.2.4) that re-fits the conformal quantile + bump + shift on each split's tune fold and verifies the deployed schedule passes Kupiec at every anchor on each split's test fold.

The 2023-01-01 split point is conservative in two respects: (i) it places the 2023 banking turbulence and the 2024–2025 macro-regime transition in the held-out slice, exposing the trained quantile to material out-of-sample regime shifts; (ii) it leaves only $\sim 245$ HOOD rows in the calibration set (HOOD IPO'd in 2021). Under v2 / M5, HOOD enters the per-regime quantile fit pooled across symbols within each regime (the trained quantile $q_r(\tau)$ does not stratify by symbol — see §4.2), so the calibration-window thinness for HOOD is absorbed into the pooled regime bin rather than producing a per-symbol fallback. The §7.2.2 M3 row (per-symbol Mondrian) confirms that further per-symbol stratification thins each cell to $N \approx 50$–$300$ and degrades Christoffersen — the deployed pooled-by-regime stratification is a deliberate sample-size choice.
