...
- **Oracle latency vs. block production.** LVR-minimising AMMs depend on the oracle being faster than the underlying market. Pyth Pro (formerly Lazer) and RedStone Bolt are racing to provide sub-second updates; the band-AMM needs to be evaluated against that timing, not against a 30s-update assumption. (Per canonical [`docs/sources/oracles/pyth_lazer.md`](../../docs/sources/oracles/pyth_lazer.md) §1.2, regular-Pyth slot cadence is ~400ms; Pyth Pro subscriber-customizable cadence is finer, with exact values not surfaced in the public overview page. RedStone's official Bolt product/docs surface advertises ~2.4ms updates, currently tied to the MegaETH path.)
...
- **Pyth integrated with Polymarket** for prediction-market settlement on RWAs (gold, oil, US equities). Pyth Data Marketplace with Fidelity Investments / Euronext FX backing was publicly announced by Pyth on 2026-04-06.
