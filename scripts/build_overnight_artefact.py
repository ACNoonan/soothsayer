...
def main() -> None:
    ...
    L = ["# Overnight calibration — first read (Phase 3)\n",
         ...
         "\n> **Phase-2 read (paper-grade bar one deferred item).** earnings_night "
         "uses scryer earnings.v2 BMO/AMC **single-gap** timing (amc@t0 or bmo@t1); "
         "σ̂ is **de-contaminated** (earnings residuals excluded from the EWMA "
         "scale pool). Coverage robustness confirmed by moving-block-bootstrap. "
         "Remaining open item: ex-dividend mornings uncorrected (yahoo/corp_actions "
         "not yet in scryer — documented limitation + wishlist item, non-blocking "
         "since coverage already holds).\n",
         "\n### Verdict\n"
