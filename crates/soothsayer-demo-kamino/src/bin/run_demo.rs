...
//! runs each scenario through both the Soothsayer-band evaluator and the
//! Kamino-style flat-band evaluator, and emits:
//!
...
//!
//! All three contain the same numbers — the scenarios are real outputs from
//! the deployed methodology, not synthetic.
//!
//! Run:
...
fn write_markdown(rows: &[Row], path: &str) -> std::io::Result<()> {
    ...
    md.push_str("# Kamino-fork demo: Soothsayer band vs flat ±300 bps\n\n");
    md.push_str(
        "*Generated by `cargo run -p soothsayer-demo-kamino --bin run_demo`. \
         Scenario panel is `data/processed/demo_kamino_scenarios.json`, \
         produced from the deployed Python Oracle. Numbers below are real \
         outputs from the methodology in Paper 1, not synthetic.*\n\n",
    );
    md.push_str(
    ...
    md.push_str(&format!(
        "## Decision divergence\n\n**{} of {} scenarios reach a different liquidation decision** under \
         the Soothsayer band than under Kamino's flat ±300 bps band, holding the \
         lending parameters constant. The divergences are concentrated in the regimes \
         where Kamino's flat band is mis-priced relative to the realised volatility \
         envelope: in `normal` it is too wide (capital efficiency loss); in `high_vol` \
         and `long_weekend` it is too tight (uncaught risk).\n\n",
        n_diverge,
        rows.len(),
    ));
    ...
    md.push_str(
        "*Kamino's band is a governance parameter. Soothsayer's band is an empirical \
         measurement.* The two bands often agree on benign positions; they diverge in \
         exactly the windows where the deployed RWA oracle stack today is most exposed \
         to either inefficient capital lockup (over-cautious lower bound in calm \
         regimes) or under-priced tail risk (over-tight lower bound in stressed regimes).\n",
    );
...
fn write_html(rows: &[Row], path: &str) -> std::io::Result<()> {
    ...
    html.push_str("  <h2>Kamino-fork demo: Soothsayer vs flat ±300 bps</h2>\n");
    html.push_str("  <p class=\"demo-kamino__caption\">Real outputs from the deployed Soothsayer methodology evaluated against a Kamino-style flat-band lending model on the same synthetic position book.</p>\n");
    ...
    let n_diverge = rows.iter().filter(|r| r.decision_diverges).count();
    html.push_str(&format!(
        "  <p class=\"demo-kamino__summary\"><strong>{} of {} scenarios</strong> reach a different liquidation decision under Soothsayer than under Kamino's flat ±300 bps. Divergence concentrates in <code>normal</code> (Kamino too wide → capital lockup) and <code>high_vol</code> / <code>long_weekend</code> (Kamino too tight → uncaught tail risk).</p>\n",
        n_diverge,
        rows.len(),
    ));
